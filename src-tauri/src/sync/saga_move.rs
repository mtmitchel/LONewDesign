...
use super::saga::{
    TaskBackup, SubtaskBackup, TaskMoveSaga,
    load_or_initialize_saga, persist_saga_state,
    acquire_lock, release_lock,
    check_or_store_idempotent_operation,
    mark_idempotent_completed, mark_idempotent_failed,
};
...
async fn execute_move_saga_internal(
    ...
    loop {
        match current_state {
            TaskMoveSaga::Initialized { task_id, from_list_id, to_list_id: _ } => {
                println!("[saga_move] Step 1: Exporting task data");
                let (task_backup, subtask_backups) = export_task_data(
                    db_pool,
                    &task_id,
                    &from_list_id,
                ).await?;
            ...
            TaskMoveSaga::TaskExported { task_backup, subtask_backups: _ } => {
                println!("[saga_move] Step 2: Deleting from source list");
                
                // Idempotent delete
                let idempotency_key = format!("delete-task-{}:{}", saga_id, &task_backup.google_id);
                delete_task_idempotent(
                    ...
                    &idempotency_key,
                    &task_backup.list_id,
                    &task_backup.google_id,
                ).await?;
                
                let next_state = TaskMoveSaga::SourceDeleted {
                    old_google_id: task_backup.google_id.clone(),
                };
                persist_saga_state(db_pool, saga_id, &next_state).await?;
                current_state = next_state;
            }
            ...
            TaskMoveSaga::SourceDeleted { old_google_id } => {
                ...
                let next_state = TaskMoveSaga::DestinationCreated {
                    new_google_id,
                };
                persist_saga_state(db_pool, saga_id, &next_state).await?;
                current_state = next_state;
            ...
            TaskMoveSaga::SubtasksCreated { ref subtask_mapping } => {
                println!("[saga_move] Step 5: Updating database");
                
                let task_backup = load_task_backup(db_pool, saga_id).await?;
                let new_google_id = match &current_state {
                    TaskMoveSaga::SubtasksCreated { .. } => {
                        // Need to find new_google_id from previous state
                        // For now, query from backup or saga logs
                        get_new_google_id_from_saga(db_pool, saga_id).await?
                    }
            ...
            _ => {
                return Err("Unexpected saga state".to_string());
            }
        }
    }
}
...
async fn export_task_data(
    ...
    // Short transaction to read data
    let mut tx = db_pool.begin().await.map_err(|e| e.to_string())?;
    
    // Load task
    let task: Option<(String, String, String, String, Option<String>, String, Option<String>, Option<String>, Option<String>)> = sqlx::query_as(
        "SELECT id, google_id, list_id, title, notes, status, due_date, position, parent_google_id 
    ...
    let task_backup = TaskBackup {
        ...
        due_date: task.6,
        position: task.7,
        parent_google_id: task.8,
    };
    
    // Load subtasks
    let subtask_records: Vec<(String, String, String, String, bool, Option<String>, Option<String>)> = sqlx::query_as(
...
async fn load_task_backup(db_pool: &SqlitePool, saga_id: &str) -> Result<TaskBackup, String> {
    let backup: Option<String> = sqlx::query_scalar(
        "SELECT task_backup_data FROM task_backups WHERE saga_id = ?"
    )
    .bind(saga_id)
...
async fn load_subtask_backups(
    ...
    let backups: Option<String> = sqlx::query_scalar(
        "SELECT subtask_backups_data FROM task_backups WHERE saga_id = ?"
    )
    .bind(saga_id)
    .fetch_optional(db_pool)
    .await
...
async fn delete_task_idempotent(
    ...
    let request_params = format!("{{\"list_id\":\"{}\",\"google_id\":\"{}\"}}", list_id, google_id);
...
async fn create_task_idempotent(
    ...
    let mut payload = serde_json::json!({
        "title": task_backup.title,
        "status": task_backup.status,
    });
    
    if let Some(notes) = &task_backup.notes {
        payload["notes"] = serde_json::json!(notes);
    }
    if let Some(due) = &task_backup.due_date {
...
async fn update_database_atomic(
    ...
    // Update task
    sqlx::query(
        "UPDATE tasks_metadata SET google_id = ?, list_id = ? WHERE id = ?"
    )
    .bind(new_google_id)
    .bind(new_list_id)
    .bind(task_id)
    ...
    for (old_subtask_id, new_subtask_google_id) in subtask_mapping {
        sqlx::query(
            "UPDATE task_subtasks SET google_id = ?, parent_google_id = ? WHERE id = ?"
        )
...
/// Get new google_id from saga state
async fn get_new_google_id_from_saga(
    db_pool: &SqlitePool,
    saga_id: &str,
) -> Result<String, String> {
    let state_json: Option<String> = sqlx::query_scalar(
        "SELECT state FROM saga_logs WHERE id = ?"
    )
    .bind(saga_id)
    .fetch_optional(db_pool)
    .await
    .map_err(|e| format!("Failed to load saga state: {}", e))?;
    
    let state_json = state_json.ok_or_else(|| "Saga not found".to_string())?;
    let _state: TaskMoveSaga = serde_json::from_str(&state_json)
        .map_err(|e| format!("Failed to deserialize state: {}", e))?;
    
    // Walk backwards through states to find DestinationCreated
    // For now, parse the JSON manually
    let value: serde_json::Value = serde_json::from_str(&state_json)
        .map_err(|e| format!("Failed to parse state JSON: {}", e))?;
    
    if let Some(data) = value.get("data") {
        if let Some(new_google_id) = data.get("new_google_id") {
            if let Some(id_str) = new_google_id.as_str() {
                return Ok(id_str.to_string());
            }
        }
    }
    
    Err("Could not find new_google_id in saga state".to_string())
}
