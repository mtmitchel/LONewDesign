    ...
    <div className={cn("space-y-3", className)}>
        ...
        <div className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center gap-1">
          ...
          {showFilterButton && filters.length > 0 && (
            <Button
              ...
              className={cn(
                "h-6 px-2 text-[var(--text-secondary)]",
                (showFilters || activeFilterCount > 0) && "text-[var(--primary)]"
              )}
            >
              <Filter size={14} />
              {activeFilterCount > 0 && (
      ...
      {activeFilterCount > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-[var(--text-secondary)]">Filters:</span>
          {Object.entries(activeFilters).map(([filterId, filterValue]) => {
            ...
            return (
              <Badge
                key={filterId}
                variant="secondary"
                className="text-xs bg-[var(--primary-tint-15)] text-[var(--primary)] hover:bg-[var(--primary-tint-20)]"
              >
          ...
          <Button
            ...
            className="h-6 px-2 text-xs text-[var(--text-secondary)] hover:text-[var(--error)]"
          >
            Clear all
          </Button>
        </div>
      ...
      {showFilters && filters.length > 0 && (
        <div className="p-4 bg-[var(--elevated)] border border-[var(--border-subtle)] rounded-lg space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-medium text-[var(--text-primary)]">Filters</h4>
            <Button
              variant="ghost"
              size="sm"
              ...
              <div key={filter.id} className="space-y-1">
                ...
                {filter.type === 'select' && filter.options && (
                  <select
                    value={activeFilters[filter.id] || ''}
                    onChange={(e) => handleFilterChange(filter.id, e.target.value)}
                    className="w-full px-3 py-1 text-sm border border-[var(--border-default)] rounded-md bg-[var(--surface)] text-[var(--text-primary)]"
                  >
