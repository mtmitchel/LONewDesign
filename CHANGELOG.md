...
### Enhancements
- **Conflict surfacing (end-to-end complete, 2025-10-22)**: Google Tasks conflict detection now wired end to end. Backend (`sync_service.rs`) marks conflicts without resetting them, blocks queued mutations via `mark_pending_queue_conflict`, and emits `tasks::conflict` events carrying remote/local snapshots plus dirty field diffs. Commands (`commands/tasks.rs`) clear conflict flags when authors re-edit tasks, letting next sync resolve cleanly. Frontend listens for the Tauri event; `taskStore.tsx` hydrates the remote payload into state, sets `syncState` to `conflict`, and preserves the badge, while `types.ts` includes the expanded sync-state enum. Remaining work: read-only store migration and Rust/TS regression tests.
